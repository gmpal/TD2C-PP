        ...
        for edge in results.edges:
            ...
            if lag > 0:  # we ignore contemporaneous relations for the moment
                ...
                causal_dataframe.loc[
                    (source + lag * n_variables, effect), "is_causal"
                ] = 1
                causal_dataframe.loc[(source + lag * n_variables, effect), "effect"] = (
                    value
                )
                causal_dataframe.loc[
                    (source + lag * n_variables, effect), "p_value"
                ] = None

        causal_dataframe.reset_index(inplace=True)

        assert (
            len([edge for edge in results.edges if "lag0" not in edge[0]])
            == causal_dataframe.is_causal.sum()
        )
