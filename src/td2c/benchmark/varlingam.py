...
class VARLiNGAM(BaseCausalInference):
    ...
    def complete_causal_df(self, causal_df, n_variables):
        ...
        all_pairs = [
            (from_, to)
            for from_ in range(n_variables, n_variables * (self.maxlags + 1))
            for to in range(n_variables)
        ]

        existing_pairs = set(zip(causal_df["from"], causal_df["to"]))
        missing_pairs = [
            (from_, to) for from_, to in all_pairs if (from_, to) not in existing_pairs
        ]
